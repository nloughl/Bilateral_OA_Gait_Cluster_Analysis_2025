---
title: "Bilateral PAM, KAMILA, PAM-MS Clustering Using MDS Coordinates"
...
Clustering is sensitive to outliers, so we preemptively check for outliers before attempting clustering. Outliers can result in poor silhouette width or ARI, making it difficult to identify the optimal number of clusters to use. 
...
cat("Does this data set have outliers?", has_outliers, "\n")
```
...
if (has_outliers) {
  ...
  # Store results
  cat("Outliers detected. The following rows were removed:", print(outliers), "\n")
  ...
  cat("No outliers detected. Proceed with original dataset.\n")
}
```
...
KAMILA will choose the optimal cluster number from 2:10 based on prediction strength and will choose the smallest k with prediction strength over the threshold
...
Evaluate cluster agreement across methods 
...
jaccard <- cboot_pam$subsetmean # mean Jaccard per cluster
dissolved <- cboot_pam$subsetbrd
recovered <- cboot_pam$subsetrecover  

mean(cboot_pam$subsetmean)  # clusterwise Jaccard mean
...
## Compare Cluster Stability Across Clustering Methods 
