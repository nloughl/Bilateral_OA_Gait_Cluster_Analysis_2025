---
title: "Bilateral PAM, KAMILA, PAM-MS Clustering with MDS"
...
dir.create("outputs/mixed_results_mds", showWarnings = FALSE)
```
...
mixed_data <- mixed_data %>%
  left_join(mds_coords %>%
              dplyr::select(Dim1, Dim2, subject, signal_side), by = c("subject", "signal_side")) 
...
Data.Cluster <- mixed_data %>%
  dplyr::select(subject, Dim1, Dim2, sex, severity_contra, knee_oa_location) %>%
  na.omit()
...
# Elbow Method 
fviz_nbclust(gower_mat, cluster::pam, method = "wss", k.max = 20)
...
ari_results_pam_aricode <- matrix(NA, nrow = n_iterations, ncol = length(k_range))
ari_results_ms_aricode  <- matrix(NA, nrow = n_iterations, ncol = length(k_range))
ari_results_kam_aricode  <- matrix(NA, nrow = n_iterations, ncol = length(k_range))
...
continuous_features <- c("Dim1", "Dim2")
...
for (i in 1:n_iterations) {
  ...
  for (j in seq_along(k_range)) {
    ...
    diss_mat <- daisy(sampled_data[, c(continuous_features, categorical_features)], metric = "gower")
...
for (i in 1:n_iterations) {
  ...
  for (j in seq_along(k_range_ms)) {
    ...
    ari_results_ms_aricode[i, j] <- ARI(full_clusters_sub_ms, ms_sub_clusters)
...
ari_sil_ami_summary <- data.frame(
  ...
  mean_ari_pam_aricode = colMeans(ari_results_pam_aricode, na.rm = TRUE),
  ...
  mean_ari_ms_aricode = colMeans(ari_results_ms_aricode, na.rm = TRUE),
  ...
  mean_ari_kam_aricode = colMeans(ari_results_kam_aricode, na.rm = TRUE),
...
stability_df <- data.frame(
  ...
  Mean_ARI = c(mean(ari_results_pam_aricode), mean(ari_results_kam_aricode), mean(ari_results_ms_aricode)),
  Mean_AMI = c(mean(ami_results_pam), mean(ami_results_kam), mean(ami_results_ms)),
  SD_ARI = c(sd(ari_results_pam_aricode), sd(ari_results_kam_aricode), sd(ari_results_ms_aricode)),
  SD_AMI = c(sd(ami_results_pam), sd(ami_results_kam), sd(ami_results_ms))
)

print(ari_sil_ami_summary)
write.csv(ari_sil_ami_summary, file="outputs/mixed_results_mds/PAM_MS_ari_sil_ami_summary.csv", row.names = FALSE)
...
numclust_pam = ari_sil_ami_summary$k[which.max(ari_sil_ami_summary$mean_ari_pam)]
numclust_kamila = ari_sil_ami_summary$k[which.max(ari_sil_ami_summary$mean_ari_kamila)]
numclust_ms = ari_sil_ami_summary$k[which.max(ari_sil_ami_summary$mean_ari_ms)]
...
write.csv(cluster_agreement, "outputs/mixed_results_mds/mixed_data_cluster_agreement.csv", row.names = FALSE)
...
# Save to CSV
write.csv(jaccard_summary_pam_extended, "outputs/mixed_results_mds/jaccard_PAM_with_overall.csv", row.names = FALSE)
```
...
# Save
write.csv(jaccard_summary_extended, "outputs/mixed_results_mds/jaccard_KAM_with_overall.csv", row.names = FALSE)
```

## PAM-MS CLuster Stability 

```{r}
bootstrap_ms_pam <- function(conData, catData, k=numclust_pam, B = 100, conWeight = 0.5) {
  # Store cluster results
  cluster_results <- list()
...
# Save to CSV
write.csv(jaccard_summary_ms_extended, "outputs/mixed_results_mds/jaccard_MS_with_overall.csv", row.names = FALSE)
```
...
Data.tables <- Data.Cluster %>%
  ...
  left_join(mds_input_data %>%
              dplyr:: select("subject", "age", "PC1_front", "PC1_sag", "PC2_sag", "PC3_sag", "speed"), by = "subject")
...
write.csv(PAM_clusters_matrix, file = "outputs/mixed_results_mds/PAM_clusters_summary.csv")
```
...
write.csv(MS_clusters_matrix, file = "outputs/mixed_results_mds/MS_clusters_summary.csv")
```
...
write.csv(KAM_clusters_matrix, file = "outputs/mixed_results_mds/KAM_clusters_summary.csv")
```
...
## Re-run clustering without outliers (if any) 
...
feature_importance <- function(data, cluster_col, cluster_name, continuous_vars, categorical_vars, test_results) {
  ...
  print(p_values)
  write.csv(p_values, file = sprintf("outputs/mixed_results_mds/feature_importance_%s.csv", cluster_name), row.names = FALSE)
  ...
  write.csv(centroid_importance, file = sprintf("outputs/mixed_results_mds/centroid_importance_%s.csv", cluster_name), row.names = FALSE)
  ...
  write.csv(categorical_importance, file = sprintf("outputs/mixed_results_mds/categorical_importance_%s.csv", cluster_name), row.names = FALSE)
...
test_continuous <- function(var, cluster_col, data, cluster_name) {
  ...
  if (any(shapiro_test$p_value < 0.05, na.rm = TRUE)) {
    ...
    write.csv(kruskal_df, file = sprintf("outputs/mixed_results_mds/%s_%s_kruskal.csv", var, cluster_name), row.names = FALSE)
  } else {
    ...
    write.csv(anova_df, file = sprintf("outputs/mixed_results_mds/%s_%s_anova.csv", var, cluster_name), row.names = FALSE)
  }
  ...
  if (is.numeric(p_value) && !is.na(p_value) && p_value < 0.05) {
    if (any(shapiro_test$p_value < 0.05, na.rm = TRUE)) {
      ...
      write.csv(dunn_df, file = sprintf("outputs/mixed_results_mds/%s_%s_dunn.csv", var, cluster_name), row.names = FALSE)
    } else {
      ...
      write.csv(tukey_df, file = sprintf("outputs/mixed_results_mds/%s_%s_tukey.csv", var, cluster_name), row.names = FALSE)
    }
  } else {
...
test_categorical <- function(var, cluster_col, data, cluster_name) {
  ...
  write.csv(as.data.frame.matrix(contingency_table), 
            file = sprintf("outputs/mixed_results_mds/%s_%s_contingency.csv", var, cluster_name), 
            row.names = TRUE)
  ...
  write.csv(chi_df, file = sprintf("outputs/mixed_results_mds/%s_%s_chi.csv", var, cluster_name), row.names = FALSE)
  ...
  if (any(chi_test$expected < 5)) {
    ...
    write.csv(fisher_df, file = sprintf("outputs/mixed_results_mds/%s_%s_fisher.csv", var, cluster_name), row.names = FALSE)
  }
...
# Run Statistical Analysis and Feature Importance on Clusters (after outlier removal)
